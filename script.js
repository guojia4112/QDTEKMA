...
function showPage(pageId){
  ...
  if(target){
    ...
    var pageNames=['diagram','evolution','sandbox','method'];
    for(var b=0;b<btns.length;b++){ btns[b].classList.remove('active'); if(pageNames[b]===pageId) btns[b].classList.add('active'); }
...
function initSandbox(){
  ...
  drawBackgroundIfMissing('sandboxPlot').then(function(){
    ...
    'https://api.trans-ekma.com/api/qd/xy2vocnox',
    {
      method:'POST',
      headers:{
  ...
  const response2 = await fetch(
    'https://api.trans-ekma.com/api/qd/vocnox2xy',
    {
      method:'POST',
      headers:{
        'Content-Type':'application/json'
